...
sub deserialize {
  ...
  local $Paludis::ResumeState::Serialization::Grammar::CLASS_CALLBACK = sub {
    my ( $classname, $parameters_list ) = @_;
    return { type => 'class', classname => $classname, parameters => $parameters_list };
  };

  local $Paludis::ResumeState::Serialization::Grammar::LIST_CALLBACK = sub {
    my ($parameters, $parameters_list) = @_;
    return { type => 'array', parameters => $parameters_list, count => scalar @{$parameters} };
  };
  local %/;
